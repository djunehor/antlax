...
$query = "select * from $user_table WHERE activated=1 AND role=0 ORDER BY userID DESC";			  
...
while($v = mysqli_fetch_array($mmm))
		...
		?>
                <tr id="user<?php echo $v['userID']; ?>">
                  ...
                  <td><?php $ref = mysqli_num_rows(mysqli_query($con,"SELECT * FROM $user_table WHERE activated=1 AND refid='".$v['userID']."' AND joindate between '".strtotime('this month')."' AND '".(strtotime('next month')-1)."'"));
echo $ref;				  ?></td>
